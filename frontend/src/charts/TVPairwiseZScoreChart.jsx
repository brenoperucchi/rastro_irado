// Z-Score Dinâmico (Pair Spread) — primeiro chart migrado para lightweight-charts.
//
// Piloto da migração Recharts → lightweight-charts: é greenfield no local (não
// existia), então construí-lo direto na lib nova custa o mesmo e não gera
// descarte. Plota `pair_z` (z-score do resíduo do par ativo do Kalman) ao longo
// da sessão, com bandas de compra/venda em ±2σ. Reconstruído do bundle de
// produção (componente TVPairwiseZScoreChart).
...
const TVPairwiseZScoreChart = forwardRef(function TVPairwiseZScoreChart(
  ...
  useEffect(() => {
    ...
    seriesRef.current.setData(buildSeriesData(history, effectiveDate, 'pair_z'))
...
export default TVPairwiseZScoreChart
